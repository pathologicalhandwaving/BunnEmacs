...
***** Plain lists have bullet shaped bullets
#+begin_src emacs-lisp
...
***** Modify demote plain list bullets
#+BEGIN_SRC emacs-lisp
...
When a checklist box =[X]= then it will be grayed out
#+begin_src emacs-lisp
...
***** Cycle will include plain lists
#+begin_src emacs-lisp
...
(add-to-list 'org-emphasis-alist
             '("*" (:foreground "#BF616A" :weight bold)
               "+" (:foreground "#ECEFF4" (:strike-through t))
               "_" (:foreground "#D08770" (:underline t) :weight bold)
               "=" (:foreground "#A3BE8C" (org-code verbatim))
               "~" (:foreground "#B48EAD" (org-verbatim verbatim))
               "/" (:foreground "#88C0D0" :slant italic)))
#+end_src
...
- 7 hours
- 9 hours
Stored in column mode in global property =EFFORT_ALL= 
#+begin_src emacs-lisp
(setq org-global-properties (quote (("EFFORT_ALL" . "00:05 00:15 00:25 00:35 00:45 00:90 02:00 03:00 05:00 07:00 09:00 00:00")
                                    ("STYLE_ALL" . "habit"))))
#+end_src

To create an estimate start column mode with =C-c C-x C-c= and collapse tree with =c=.
This shows a table overlay on top of the headlines with task name, effort estimate and clocked time in columns.

Place cursor in =Effort= column for a task to set the estimated effort value with keys 1 - 9.
Exit column mode with =q=

Entering =C-c C-x i RET= inserts a clock table report with your estimated values and any clocked time to date.

Quick summaary of clocked time in current org buffer =C-c C-x C-d=
...
*** Habits
Habit is regular task with special property setting it as repeated
#+begin_example
 * TODO Take vitamins
   - SCHEDULED: <2020-08-08 .+7d/30d>
   :PROPERTIES:
   :STYLE: habit
   :END:
#+end_example
...
**** Redisplay habits in agenda each day
#+begin_src emacs-lisp
(run-at-time "06:00" 86400 '(lambda () (setq org-habit-show-habits t)))
#+end_src

**** Logging Habits
Logging only logs changes to DONE state
Allows cancelling habit without recording a timestamp as this messes up the habit graph.
Cancelling a habit to remove it from agenda because it is not possible today should not mark the habit as done.

Example of habit logging
#+begin_example
 * Habits
   :PROPERTIES:
   :LOGGING: DONE
...
#+begin_src emacs-lisp
(setq org-clock-in-switch-to-state 'bun-clock-into-next)
#+end_src
***** TODO =bun-clock-into-next= function
...
**** Save clock data and state changes and notes to LOGBOOK drawer 
#+begin_src emacs-lisp
(setq org-clock-into-drawer t)
#+end_src

**** Clock out when task moves to DONE state
#+begin_src emacs-lisp
...
**** Save running clock and all clock history when exiting Emacs, load on startup
#+begin_src emacs-lisp
(setq org-clock-persist t)
#+end_src

**** Do Not prompt to resume active clock
#+begin_src emacs-lisp
...
**** Clock report summarize time spent
#+begin_src emacs-lisp
...
**** =bun-clock-into-next=
#+begin_src emacs-lisp
(defun bun-clock-into-next (kw)
 "Switch a task from TODO to NEXT when clocking in.
  Skips capture tasks, projects, and subprojects.
  Switch project and subprojects from NEXT back to TODO."
...
**** =bun-find-project-task=
#+begin_src emacs-lisp
(defun bun-find-project-task ()
  ...
  (if (equal major-mode 'org-agenda-mode)
      ...
      (let* ((marker (org-get-at-bol 'org-hd-marker))
        ...
        (if (and (eq arg 4) tags)
          ...
          (bun-clock-in-organization-task-as-default)))
      ;; we are NOT in agenda
      (save-restriction
        (widen)
...
(defun bun-clock-out ()
  (interactive)
  (setq bun-keep-clock-running nil)
  (when (org-clock-is-active)
    (org-clock-out))
  (org-agenda-remove-restriction-lock))
#+end_src

**** =bun-clock-in-default-task=
#+begin_src emacs-lisp
(defun bun-clock-in-default-task ()
  (save-excursion
    (org-with-point-at org-clock-default-task
      (org-clock-in))))
#+end_src

**** =bun-clock-in-parent-task=
#+begin_src emacs-lisp
(defun bun-clock-in-parent-task ()
      ...
      (save-restriction
        ...
        (if parent-task
          (org-with-point-at parent-task
            (org-clock-in))
          (when bun-keep-clock-running
            (bun-clock-in-default-task)))))))
#+end_src

**** TODO =bun-organization-task-id=
#+begin_src emacs-lisp
(defvar bun-organization-task-id "")
#+end_src

**** =bun-clock-in-organization-task-as-default=
#+begin_src emacs-lisp
(defun bun-clock-in-organization-task-as-default ()
  (interactive)
  (org-with-point-at (org-id-find bun-organization-task-id 'marker)
    (org-clock-in '(16))))
#+end_src

**** =bun-clock-out-maybe=
#+begin_src emacs-lisp
(defun bun-clock-out-maybe ()
  (when (and bun-keep-clock-running
             (not org-clock-clocking-in)
             (marker-buffer org-clock-default-task)
             (not org-clock-resolving-clocks-due-to-idleness))
     (bun-clock-in-parent-taks)))
#+end_src

#+begin_src emacs-lisp
(add-hook 'org-clock-out-hook 'bun-clock-out-maybe 'append)
#+end_src
...
**** =bun-clock-in-task-by-id=
#+begin_src emacs-lisp
(defun bun-clock-in-task-by-id (id)
 "Clock in task by ID"
 (org-with-point-at (org-id-find id 'marker)
   (org-clock-in nil)))
#+end_src

**** =bun-clock-in-last-task=
#+begin_src emacs-lisp
(defun bun-clock-in-last-task (arg)
 "Clock in the interrupted task if there is one. Skip the default task and get the next one. A prefix arg forces clock in of the default task."
  (interactive "p")
...
**** Discrete clocking intervals
#+begin_src emacs-lisp
(setq org-time-stamp-rounding-minutes (quote (1 1)))
#+end_src

**** One minute clocking gaps
#+begin_src emacs-lisp
(setq org-agenda-clock-consistency-checks
  (quote (:max-duration "04:00"
          :min-duration 0
          :max-gap 0
          :gap-ok-around ("04:00"))))
#+end_src
