...
*BunnEmacs* is a MacOS centric emacs distribution, developed on MacOS Catalina 10.15.6 using the development version of *Emacs* 28 HEAD (see README for installation steps). BunnEmacs is highly opinionated and specific to MacOS.

By default it launches emacsclient using a [[https://www.hammerspoon.org/][Hammerspoon]] keybinding (see details in README).

BunnEmacs is still in development but works great so far! Suggestions welcome!

BunnEmacs use-cases are focused on note taking, academic style research and development work with heavy use of embedded LaTeX code and shell scripts.
